...
test.admin(
  'US13 [Admin to Admin] | Edit Admin/Staff Profile',
  async ({ page }, testInfo) => {
    ...
    await test.admin.step(
      'Expect redirected to Staff Details Page',
      async () => {
    ...
    await test.admin.step('Edit staff profile', async () => {
      await page.getByRole('button', { name: 'Edit' }).click()
      await expect.soft(page.locator('#PopupRegisterAdmin')).toContainText(
        'Edit Staff Registration'
      )
      await expect.soft(
        page.getByPlaceholder('Name of Staff as in NRIC')
      ).toHaveValue(staffName)
      ...
      let staffType = await page.getByLabel('Staff Type').inputValue()
      await page.getByLabel('Staff Type').selectOption(staffType == '0' ? '1' : '0')
    ...
    await test.admin.step('Expect admin profile has updated', async () => {
      ...
      await expect(page.locator('#b11-MainContainer')).toContainText('No records found...');
    })
  }
)
