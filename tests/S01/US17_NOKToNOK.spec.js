...
test('US17 [NOK to NOK] | Access Login Screen', async ({
  page,
}, testInfo) => {
  // LOGIN
...
test('US17 [NOK to NOK] | Resend OTP', async ({ page }, testInfo) => {
  // LOGIN
  await test.step('Input Mobile Number', async () => {
...
test('US17 [NOK to NOK] | Enter OTP and Login', async ({
  page,
}, testInfo) => {
  // LOGIN
  await test.step('Input Mobile Number', async () => {
...
test('US17 [NOK to NOK] | Incorrect OTP Code', async ({ page }, testInfo) => {
  // LOGIN
  await test.step('Input Mobile Number', async () => {
    // go to Admin login page
    await page.goto(NOK_BASE_URL)
  ...
  await test.step('Input Invalid OTP Code and Login', async () => {
    ...
    await page.getByRole('button', { name: 'Verify OTP' }).click()
    await expect(page.locator('#InputOTP_DescribedBy')).toContainText(
      'Invalid OTP Code'
    )
...
// test('US17 [NOK to NOK] | Excessive OTP Attempts', async ({
//   page,
// }, testInfo) => {
//   // LOGIN
//   await test.step('Input Mobile Number', async () => {
