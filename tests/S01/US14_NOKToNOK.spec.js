...
test('US14 [NOK to NOK] | Access My Profile', async ({
  page,
}, testInfo) => {
  // LOGIN
  ...
  await test.step('Access NOK My Profile Page', async () => {
    await page.goto('https://outtst.werkdone.com/NOKGatewayPortal_UI/NOK_MyProfile')
...
test('US14 [NOK to NOK] | Profile Data Accuracy', async ({
  ...
  await test.step('Expect My Profile has correct data', async () => {
    expect(await page.locator('#Input_FullName').inputValue()).toBe('NOK 01')
    expect(await page.locator('#Input_MobileNumber').inputValue()).toBe(NOK_PHONE_NUMBER)
    await page.close()
  })
})